...
import { resolveInitialPageFromPath } from '../lib/helpers';
...
export function AppStateProvider({ children }: { children: React.ReactNode }) {
  const [currentPage, setCurrentPage] = useState<string>('admin-login');
  ...
  const stripSensitiveBooking = (booking: unknown) => {
    ...
    const {
      firstName: _firstName, lastName: _lastName, email: _email, phone: _phone,
      phoneNumber: _phoneNumber, cardNumber: _cardNumber, cvv: _cvv,
      expiryDate: _expiryDate, cardName: _cardName, ...safe
    ...
    const loadState = () => {
      try {
        ...
        if (stored) {
          ...
          if (parsed.agencyId) setAgencyId(parsed.agencyId);
          ...
          if (parsed.userRole && parsed.userRole !== 'guest') {
  ...
  }, [isLoading, searchParams, selectedTrip, selectedSeats, baggageData, bookingData, paymentStatus, userPreferences, sessionTimestamp, agencyId, userRole]);
  ...
  const completeBooking = useCallback((paymentData: BookingPaymentData) => {
  ...
  const recoverSession = useCallback(() => {
    if (bookingData) setCurrentPage('confirmation');
    else if (baggageData) setCurrentPage('payment');
    else if (selectedSeats.length > 0) setCurrentPage('baggage');
    else if (selectedTrip) setCurrentPage('seats');
    else if (searchParams) setCurrentPage('search');
  }, [bookingData, baggageData, selectedSeats, selectedTrip, searchParams]);

  const getSessionAge = useCallback(() => Date.now() - sessionTimestamp, [sessionTimestamp]);
  ...
  const clearHistory = useCallback(() => setBookingHistory([]), []);

  const value: AppState = {
    currentPage, isLoading, canGoBack, isAuthenticated,
    searchParams, selectedTrip, selectedSeats, baggageData,
    bookingData, bookingProgress, paymentStatus, bookingHistory,
    favoriteRoutes, userPreferences, userRole, agencyId,
    setCurrentPage, setSearchParams, setSelectedTrip, setSelectedSeats,
    setBaggageData, completeBooking, resetBookingFlow, recoverSession,
    getSessionAge, addToFavorites, updatePreferences, clearHistory,
    setUserRole, setAgencyId, setIsAuthenticated,
  };
