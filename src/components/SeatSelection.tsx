...
export function SeatSelection({ trip, passengers, onContinue, onBack, preferences }: SeatSelectionProps) {
      ...
      if (wagon.layout === "compartment") {
        ...
        for (let comp = 1; comp <= compartments; comp++) {
          ...
          for (let pos = 0; pos < 4; pos++) {
            ...
            seats.push({
              id: `W${wagonNumber}C${comp}${positions[pos]}`,
              row: comp,
  ...
  }, [trip, isTrainTrip, selectedWagon]);
    ...
    if (selectedSeats.find(s => s.id === seat.id)) {
      ...
      if (selectedSeats.length >= passengers) {
        toast.warning('Limite atteinte', {
          description: `Maximum ${passengers} ${isTrainTrip ? 'place' : 'siège'}${passengers > 1 ? 's' : ''}`
        });
        return;
      }
      ...
      const seatName = seat.type === 'couchette' ? `Compartiment ${seat.compartment}-${seat.column}` : `${seat.row}${seat.column}`;
      toast.success(`${seatName} sélectionné - ${formatPrice(seat.price)}`);
    }
  };
                  ...
                  <CardContent className="p-4">
                    {(() => {
                      ...
                      return (
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            <div className="text-h6 text-kongo-black font-semibold">
                              {seat.type === 'couchette' 
            ...
            <Card className="card-elevated">
              <CardHeader>
                <CardTitle className="text-h5 text-kongo-black flex items-center">
                  ...
                  <Badge className="ml-2 status-kongo">
              ...
              <CardContent className="space-y-4">
                {selectedSeats.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <div className="text-body-small text-secondary">
                      Sélectionnez {passengers} {isTrainTrip ? 'place' : 'siège'}{passengers > 1 ? 's' : ''} pour continuer
                    </div>
                  </div>
                ) : (
                  <div className="space-y-3">
