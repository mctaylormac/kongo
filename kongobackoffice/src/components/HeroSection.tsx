...
export function HeroSection({ onSearch }: HeroSectionProps) {
  ...
  useEffect(() => {
    ...
    // Fetch locations from dynamic table
    fetchLocations();
  }, []);
  ...
  useEffect(() => {
    const fetchStops = async () => {
      ...
      const { data } = await supabase.from('stops').select('*').ilike('city_name', `%${fromLocation}%`);
  ...
  useEffect(() => {
    const fetchStops = async () => {
      ...
      const { data } = await supabase.from('stops').select('*').ilike('city_name', `%${toLocation}%`);
  ...
  const fetchLocations = async () => {
    try {
      ...
      const uniqueLocations = data?.reduce((acc: any[], current: any) => {
      ...
      // Fallback to initial hardcoded list if database fails or is empty
      setDbLocations([
        { id: '1', name: 'Kinshasa' },
            ...
            >
              <div className="text-center">
                <p className="text-body-small text-secondary mb-4">
                  <span className="font-semibold">Raccourcis rapides</span> - Lancez une recherche pré-remplie
                </p>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <motion.button
                        ...
                        className={`h-12 rounded-lg border-2 flex flex-col items-center justify-center space-y-1 transition-all duration-200 ${transportType === "bus"
                      ...
                      <motion.button
                        ...
                        className={`h-12 rounded-lg border-2 flex flex-col items-center justify-center space-y-1 transition-all duration-200 ${transportType === "train"
                          ? 'border-kongo-lime bg-surface-kongo-lime-light text-kongo-lime-dark'
                          : 'border-border-secondary hover:border-kongo-lime/50 bg-surface-primary'
                          }`}
                      >
                      ...
                      <motion.button
                        ...
                        className={`h-12 rounded-lg border-2 flex flex-col items-center justify-center space-y-1 transition-all duration-200 ${transportType === "all"
                          ? 'border-kongo-lime bg-surface-kongo-lime-light text-kongo-lime-dark'
                          : 'border-border-secondary hover:border-kongo-lime/50 bg-surface-primary'
                          }`}
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={passengers} onValueChange={setPassengers}>
                        ...
                        <SelectContent>
                          {/* Extension limite passagers à 50 pour groupes */}
                          {[...Array(50)].map((_, i) => (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <label className="text-label text-kongo-black flex items-center space-x-2">
                        ...
                        <span className="font-semibold">Arrêt de départ (Optionnel)</span>
                      </label>
                    ...
                    <div className="space-y-2">
                      <label className="text-label text-kongo-black flex items-center space-x-2">
                        <MapPin className="w-4 h-4 text-error/70" />
                        <span className="font-semibold">Arrêt d'arrivée (Optionnel)</span>
                      </label>
