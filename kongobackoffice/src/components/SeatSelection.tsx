...
export function SeatSelection({ trip, passengers, onContinue, onBack, preferences }: SeatSelectionProps) {
  ...
  // [Agent Caissier] - Action: Ajout séparation réservé (en attente) vs occupé (payé)
  const [dbReservedSeats, setDbReservedSeats] = useState<string[]>([]);
  
  const isTrainTrip = trip?.vehicleType === 'train' || trip?.trainType;

  // [Agent Caissier] - Action: Fetch corrigé en 2 étapes — évite le bug Supabase sur les filtres de joins
  // Distingue ROUGE (occupé/payé) et ORANGE (réservé/en attente)
  useEffect(() => {
    if (!trip?.id) return;
    
    const fetchOccupiedSeats = async () => {
      try {
        // Étape 1 : Récupérer tous les bookings actifs du voyage
        ...
        // Étape 2a : Sièges occupés (confirmés/payés)
        ...
        // Étape 2b : Sièges réservés (en attente de paiement)
    ...
    // Temps réel : écouter les deux tables
    const channel = supabase
      .channel(`seats_updates_cashier_${trip.id}`)
      .on('postgres_changes',
  ...
  useEffect(() => {
    ...
    const generateBusLayout = () => {
      const seats: Seat[] = [];
      const totalRows = Math.ceil((trip.total_seats || 45) / 4);
      const columns = ['A', 'B', 'C', 'D'];
      let seatCount = 0;
      
      for (let row = 1; row <= totalRows; row++) {
        for (let colIndex = 0; colIndex < 4; colIndex++) {
          if (seatCount >= (trip.total_seats || 45)) break;
          ...
          // [Agent Caissier] - Action: Vérifie aussi les réservations en attente
          const isReserved = !isOccupied && dbReservedSeats.includes(seatId);
              ...
              <CardHeader className="border-b border-border-primary">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    variant="default"
                    className="bg-gray-50 text-gray-500 border border-gray-200 hover:border-black hover:text-black transition-all text-body-small h-9 px-4 rounded-lg"
                  >
                                ...
                                return (
                                  <motion.button
                                    ...
                                    className={`w-10 h-10 rounded-lg border-2 flex items-center justify-center
                                      transition-all duration-200 relative ${getSeatColor(seat)}
                                      ${isHovered && !isOccupied ? 'ring-2 ring-kongo-lime ring-opacity-50' : ''}`}
                                    disabled={isOccupied || seat.status === 'reserved'}
                                  >
                                    {isOccupied ? (
                                      ...
                                      <>
                                        <IconComponent className="w-4 h-4" />
                                        <div className="absolute -top-1 -right-1 w-3 h-3 bg-surface-elevated rounded-full border border-border-primary flex items-center justify-center">
                                          <span className="text-[8px] leading-none">{seat.column}</span>
                                        </div>
          ...
          >
            ...
            <div className="space-y-3">
              <Button
                ...
                className="w-full h-14 bg-primary text-primary-foreground hover:bg-primary/90 font-bold rounded-xl shadow-xl transition-all disabled:opacity-50"
              >
                <CreditCard className="w-4 h-4 mr-2" />
                {selectedSeats.length === 0
                  ? `Sélectionnez au moins 1 ${isTrainTrip ? 'place' : 'sièges'}`
                  : `Procéder au Paiement (${selectedSeats.length} ${isTrainTrip ? 'place' : 'siège'}${selectedSeats.length > 1 ? 's' : ''})`}
              </Button>
              
              <Button
                onClick={onBack}
                className="w-full bg-primary text-primary-foreground hover:bg-primary/90 h-14 rounded-xl shadow-lg font-bold"
              >
