        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[420px] p-0 overflow-hidden">
                <div className="p-4 pb-3">
                    <DialogHeader>
                        <DialogTitle className="text-base">{initialData ? "Modifier l'Agence" : "Ajouter une Agence"}</DialogTitle>
                    ...
                    <div className="grid gap-2 py-2">
                        ...
                        <div className="grid gap-2 border-t pt-4 mt-2">
                            ...
                            <div className="flex flex-col gap-3">
                                <div className="flex items-center gap-3">
                                    <Input
                                        ...
                                        className="cursor-pointer h-8 py-1 text-xs"
                                    />
                                    {logoFile && (
                                        <Button 
                                            variant="ghost" 
                            ...
                            {(logoFile || formData.logoUrl) && (
                                <div className="mt-1 h-20 w-full rounded-xl border border-dashed border-border-primary overflow-hidden bg-surface-secondary flex flex-col items-center justify-center p-2">
                ...
                <div className="p-4 pt-2">
                    <DialogFooter className="gap-2">
                        <Button variant="outline" size="sm" className="h-8 px-3 text-xs" onClick={onClose} disabled={isLoading}>Annuler</Button>
                        <Button
                            size="sm"
                            onClick={handleSave}
                            disabled={isLoading}
                            className="h-8 px-3 text-xs bg-kongo-black text-white hover:bg-kongo-black/90"
                        >
...
export function AddExtraServiceForm({ isOpen, onClose, agencyId, initialData }: BaseFormProps) {
    const [isLoading, setIsLoading] = useState(false);
    const [formData, setFormData] = useState({
        title: '',
        description: '',
    ...
    useEffect(() => {
        if (initialData) {
            setFormData({
                ...
                min_weight: initialData.min_weight?.toString() || '0',
                max_weight: initialData.max_weight?.toString() || '',
    ...
    const handleSave = async () => {
        if (!formData.title || !formData.price) {
            toast.error("Le titre et le prix sont requis");
            return;
        }
        ...
        try {
            const payload = {
                ...
                sub_category: formData.category === 'baggage' ? formData.sub_category : null,
                min_weight: formData.category === 'baggage' ? parseFloat(formData.min_weight) : 0,
                max_weight: formData.category === 'baggage' && formData.max_weight ? parseFloat(formData.max_weight) : null,
                icon: formData.icon,
            ...
            let error;
            if (initialData?.id) {
                const { error: err } = await supabase.from('extra_services').update(payload).eq('id', initialData.id);
                error = err;
            } else {
                const { error: err } = await supabase.from('extra_services').insert([payload]);
                error = err;
            }

            if (error) throw error;
            toast.success(initialData ? "Service mis Ã  jour" : "Nouveau service ajoutÃ©");
            onClose();
            window.dispatchEvent(new CustomEvent('refresh-extra-services'));
        } catch (error: any) {
            toast.error("Erreur: " + error.message);
        } finally {
            setIsLoading(false);
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[460px]">
                <DialogHeader>
                    <DialogTitle>{initialData ? "Modifier" : "Ajouter"} un Service</DialogTitle>
                    <DialogDescription>Services optionnels et frais bagages</DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-2">
                    <div className="grid gap-2">
                        <Label>Titre du Service</Label>
                        <Input value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} placeholder="Ex: Assurance Voyage" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <div className="grid gap-2">
                            ...
                            <Select value={formData.category} onValueChange={val => setFormData({ ...formData, category: val })}>
                                ...
                                <SelectContent>
                                    <SelectItem value="service">Service GÃ©nÃ©ral</SelectItem>
                                    <SelectItem value="baggage">Bagage / Fret</SelectItem>
                        ...
                        <div className="grid gap-2">
                            ...
                            <Input type="number" value={formData.price} onChange={e => setFormData({ ...formData, price: e.target.value })} placeholder="5.00" />
                        </div>
                    </div>
                            ...
                            <div className="grid gap-2">
                                ...
                                <Select value={formData.sub_category} onValueChange={val => setFormData({ ...formData, sub_category: val })}>
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem value="soute">Soute</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                            <div className="grid gap-2">
                    ...
                    <div className="grid gap-2">
                        ...
                        <Textarea value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} placeholder="DÃ©crivez briÃ¨vement le service..." />
                    </div>
                </div>
                <DialogFooter>
                    <Button variant="outline" onClick={onClose} disabled={isLoading}>Annuler</Button>
