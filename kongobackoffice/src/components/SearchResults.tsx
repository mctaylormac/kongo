...
export function SearchResults({ searchParams, onSelectTrip, onSearch, userRole, onNavigateToLogin }: SearchResultsProps) {
  ...
  useEffect(() => {
    ...
    // Filtre sur le nombre de passagers : vérifier que le nombre de places disponibles est suffisant
    if (currentSearchParams?.passengers > 0) {
      filtered = filtered.filter(trip => {
        const available = trip.availableSeats !== undefined && trip.availableSeats !== null ? trip.availableSeats : trip.totalSeats;
        return available >= currentSearchParams.passengers;
      });
    }
  ...
  }, [trips, priceRange, selectedRegion, selectedServiceLevel, premiumOnly, minRating, selectedCompanies, selectedAmenities, departureTimeRange, sortBy, selectedTransportTypes, currentSearchParams?.passengers]);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={formPassengers} onValueChange={(value: string) => { setFormPassengers(value); setFormError(null); }}>
                  ...
                  <SelectContent>
                    {[...Array(50)].map((_, i) => (<SelectItem key={i + 1} value={(i + 1).toString()}>{i + 1} {i === 0 ? 'passager' : 'passagers'}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
