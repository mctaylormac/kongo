...
export function AgencyDetails({ agency, isOpen, onClose }: AgencyDetailsProps) {
  ...
  }, [agency?.id, agency?.rating, enrichedAgency.recentReviews]);

  useEffect(() => {
    if (isOpen && agency) {
      fetchReviews();

      // Simuler le comptage des vues
  ...
  const handleToggleAddReview = async () => {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
                    ...
                    <Card className="card-elevated">
                      <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        ...
                        <Button
                          ...
                          className="bg-[#5CB338] text-white hover:bg-[#4ea22e] font-bold shadow-md transition-all"
                        >
                          {showAddReviewForm ? "Fermer le formulaire" : "✍️ Laisser un avis"}
                        </Button>
                      </CardHeader>
                      <CardContent>
                          ...
                          >
                            ...
                            <div className="flex justify-end gap-3 pt-2">
                              ...
                              <Button
                                ...
                                className="bg-[#1D1D1F] text-white hover:bg-black/80 font-bold rounded-xl px-6"
                              >
                                {submittingReview ? "Publication..." : "Envoyer l'avis"}
                              </Button>
                            </div>
                          </motion.form>
                        )}
