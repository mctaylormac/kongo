...
export default function App() {
  ...
  const [notifications, setNotifications] = useState([
    {
      ...
      message: 'Profitez de 10% de r�duction sur votre premier voyage',
      timestamp: new Date(),
      read: false,
    ...
    },
    {
      ...
      message: 'D�couvrez nos nouvelles destinations vers Goma et Bukavu',
  ...
  const handleLogout = () => {
    ...
    toast.info('Deconnexion reussie', { description: 'A bientot sur KonGO !' });
  };
  ...
  // Auth Guard: Protéger et rediriger vers le dashboard approprié selon le rôle
  useEffect(() => {
    const role = appState.userRole;
    const page = appState.currentPage;
    const isAuthPage = page === NAVIGATION_PAGES.LOGIN || page === NAVIGATION_PAGES.SIGNUP;
    const isAdminRole = ['superuser', 'agency', 'chef', 'driver', 'cashier'].includes(role);

    if (isAdminRole) {
      // Rediriger vers le dashboard si l'admin essaie d'accéder aux pages publiques ou à la page de login admin
      if (!page.startsWith('admin') || page === NAVIGATION_PAGES.ADMIN_LOGIN) {
        appState.setCurrentPage(NAVIGATION_PAGES.ADMIN_DASHBOARD);
      }
    } else {
      // Un utilisateur normal (ou invité) essaie d'accéder aux pages admin (hors login admin) -> vers login admin
      if (page.startsWith('admin') && page !== NAVIGATION_PAGES.ADMIN_LOGIN) {
        appState.setCurrentPage(NAVIGATION_PAGES.ADMIN_LOGIN);
      }
    }
  }, [appState.userRole, appState.currentPage, appState.setCurrentPage]);
  ...
  useEffect(() => {
    ...
    const handleNavigateToAdmin = () => {
      appState.setCurrentPage(NAVIGATION_PAGES.ADMIN_LOGIN);
      toast.info("Accès Administration", {
        description: "Veuillez vous identifier avec vos accès privilégiés"
      });
    };
  ...
  useEffect(() => {
    const handleKeyboardShortcuts = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Escape' && appState.canGoBack) {
        ...
        toast.info("Retour", { duration: 1500 });
      }
    };
  ...
  useEffect(() => {
    if (appState.userRole === 'guest') {
      if (appState.currentPage === NAVIGATION_PAGES.DASHBOARD || appState.currentPage === NAVIGATION_PAGES.ADMIN_DASHBOARD) {
        toast.info("Veuillez vous connecter d'abord");
        appState.setCurrentPage(NAVIGATION_PAGES.LOGIN);
      }
    }
  }, [appState.currentPage, appState.userRole, appState]);
  ...
  return (
    ...
    >
      <div className="min-h-screen bg-surface-primary relative overflow-x-hidden">
        ...
        {!appState.currentPage.startsWith('admin') && (
            ...
            <main className="min-h-screen relative">
              ...
              {appState.currentPage !== NAVIGATION_PAGES.HOME && (
                <motion.div
                  ...
                  className="fixed bottom-32 right-6 z-40 hidden lg:block"
                >
              ...
              switch (action) {
                ...
                case 'feedback':
                  toast.success("Merci pour votre retour !", {
                    description: "Votre avis nous aide � am�liorer KonGO"
                  });
                  break;
              }
            }}
        ...
        {typeof process !== 'undefined' && process.env.NODE_ENV === 'development' && (
          <div className="fixed bottom-2 left-2 z-[100] bg-kongo-black text-on-black px-2 py-1 rounded text-xs font-mono">
            Page: {appState.currentPage} | Online: {connectionStatus ? 'OK' : 'OFF'}
          </div>
        )}
      </div>
    </ErrorBoundary>
