          ...
          {details && (
            ...
            >
              <div className="text-body-xs text-error bg-error/5 p-4 rounded-lg overflow-auto max-h-32">
                ...
                <br />
                <strong>Stack:</strong> {err.stack?.slice(0, 200)}...
              </div>
            </motion.details>
          )}
...
export function ConnectionStatusBanner({ isOnline, onRetry }: { isOnline: boolean; onRetry?: () => void }) {
  ...
  return (
    <motion.div
      ...
      exit={{ y: -50, opacity: 0 }}
      className="fixed top-0 left-0 right-0 bg-warning text-white p-3 text-center z-[60] shadow-lg flex items-center justify-center gap-4"
    >
      ...
      {onRetry && (
        <button 
          onClick={onRetry} 
          className="text-xs font-bold underline hover:text-white/80"
        >
          Réessayer
        </button>
      )}
...
export function ConnectionStatusIndicator({ isOnline, onRetry }: { isOnline: boolean; onRetry?: () => void }) {
  if (!isOnline) return null;
